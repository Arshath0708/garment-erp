...
abstract class DocumentFormatRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            /*
             * Columns arrive keyed by column key. Every standard key is posted
             * whether ticked or not, so a column switched off keeps the label
             * the user typed against it.
             */
            ...
            // Added by the user. Labels only — keys are derived from them.
            'custom_columns'   => ['nullable', 'array', 'max:10'],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            /*
             * A key that is not in STANDARD did not come from our form. Posting
             * one would create a column no renderer knows how to fill.
             */
            foreach (array_keys($columns) as $key) {
                if (! array_key_exists($key, DocumentFormatColumn::STANDARD)) {
                    $validator->errors()->add('columns', 'Unknown column: '.e($key));
                }
            }

            /*
             * Every column switched off, and every custom one blank, leaves an
             * item table with nothing in it but the serial number, quantity and
             * amount that are always drawn.
             */
            $enabled = collect($columns)->filter(fn ($column) => filled($column['enabled'] ?? null))->count();
            $custom  = collect($this->input('custom_columns', []))->filter(fn ($label) => filled(trim((string) $label)))->count();
